...
app.use((req, res, next) => {
  res.setHeader(
    ...
    "default-src 'self'; img-src 'self' https: data:; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; font-src 'self' data:; connect-src 'self' https: ws: wss: ws://localhost:5433;"
  );
  next();
});
